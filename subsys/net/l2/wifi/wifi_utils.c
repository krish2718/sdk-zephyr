...
{
	uint16_t valid_5g_chans[] = {32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 96, 100, 104, 108,
		112, 116, 120, 124, 128, 132, 136, 140, 144, 149, 153, 157, 159, 161, 163, 165,
...
static int wifi_utils_get_all_chans_in_range(uint16_t chan_start,
		uint16_t chan_end,
		uint16_t chan[__WIFI_FREQ_BAND_AFTER_LAST][WIFI_CHANNEL_MAX],
		uint8_t band_idx,
		uint8_t *chan_idx)
{
	uint16_t i = 0;
	...
	case WIFI_FREQ_BAND_5_GHZ:
		...
		for (i = 0; i < ARRAY_SIZE(valid_5g_chans); i++) {
			if (valid_5g_chans[i] == chan_start) {
				start = true;
			}

			if (valid_5g_chans[i] == chan_end) {
				end = true;
			}

			if (start) {
				chan[band_idx][idx++] = valid_5g_chans[i];
			}
			       ...
			       uint16_t chan[__WIFI_FREQ_BAND_AFTER_LAST][WIFI_CHANNEL_MAX])
{
	...
	while (scan_chan_str[i] != '\0') {
		if (scan_chan_str[i] == ':') {
			if ((i - band_str_start_idx) > WIFI_UTILS_MAX_BAND_STR_LEN) {
			...
			while (1) {
				...
				if (chan_start) {
					wifi_utils_get_all_chans_in_range(chan_start,
									  chan_val,
									  chan,
									  band,
									  &chan_idx);
