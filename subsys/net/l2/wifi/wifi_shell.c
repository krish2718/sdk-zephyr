				    ...
				    bool *do_scan)
{
	...
	BUILD_ASSERT(IS_ENABLED(CONFIG_SHELL_GETOPT) &&
		     ...
		     "getopt_long support is mandatory for Wi-Fi shell");
...
SHELL_STATIC_SUBCMD_SET_CREATE(wifi_commands,
	...
	SHELL_CMD(scan, NULL,
		  "Scan for Wi-Fi APs\n"
		    "OPTIONAL PARAMETERS:\n"
		    "[-t, --type <active / passive>] : Preferred mode of scan. The actual mode of scan can depend on factors such as the Wi-Fi chip implementation, regulatory domain restrictions etc. Default type is active.\n"
		    "[-b, --bands <Comma separated list of band values (2/5/6)>] : Bands to be scanned where 2: 2.4 GHz, 5: 5 GHz, 6: 6 GHz.\n"
		    "[-a, --dwell_time_active <val_in_ms>] : Active scan dwell time (in ms) on a channel. Range 5 - 1000 ms.\n"
		    "[-p, --dwell_time_passive <val_in_ms>] : Passive scan dwell time (in ms) on a channel. Range 10 - 1000 ms.\n"
		    "[-s, --ssid <Comma separate list of SSIDs>] : SSID list to scan for.\n"
		    "[-m, --max_bss <val>] : Max BSSIDs to scan for. Range 1 - 65535.\n"
		    "[-c, --chan <Comma separated list of channel ranges>] : Channels to be scanned. The channels have to be specified in the form band1:chan1,chan2_band2:chan3,..etc. The chan has to be a list of comma separated values where each value is either a single channel or a channel range specified as chan_start-chan_end. Each band channel set has to be separated by a _. E.g. a valid channel specification can be 2:1,6-11,14_5:36,149-165,44\n"
